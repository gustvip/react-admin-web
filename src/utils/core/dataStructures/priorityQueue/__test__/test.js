...
describe('PriorityQueue', () => {
	...
	it('should insert items to the queue and respect priorities', () => {
		...
		expect(priorityQueue.minHeap.peek().value).toBe(10);
		
		priorityQueue.add(5, 2);
		expect(priorityQueue.minHeap.peek().value).toBe(10);
		
		priorityQueue.add(100);
		expect(priorityQueue.minHeap.peek().value).toBe(100);
		
		priorityQueue.remove(200);
		expect(priorityQueue.minHeap.peek().value).toBe(100);
		
		priorityQueue.remove(100);
		expect(priorityQueue.minHeap.peek().value).toBe(10);
		priorityQueue.remove(10);
		expect(priorityQueue.minHeap.peek().value).toBe(5);
		priorityQueue.remove(5);
		expect(priorityQueue.minHeap.peek()).toBeUndefined();
	});
	...
	it('should poll from queue with respect to priorities', () => {
		...
		expect(priorityQueue.minHeap.poll().value).toBe(10);
	...
	it('should be possible to change priority along with node addition', () => {
		...
		priorityQueue.changePriority(300, 10);
		priorityQueue.changePriority(200, 10);
		priorityQueue.changePriority(10, 20);
		...
		expect(priorityQueue.minHeap.poll().value).toBe(5);
		expect(priorityQueue.minHeap.poll().value).toBe(200);
		expect(priorityQueue.minHeap.poll().value).toBe(15);
		expect(priorityQueue.minHeap.poll().value).toBe(10);
	});
