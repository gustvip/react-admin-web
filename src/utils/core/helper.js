...
class Helper {
	 ...
	 * @return {*}
	 */
	immutable (x, cb, enumerable = true) {
		const _this = this
		/**
		 * 回掉函数的处理
		 * 不加会将其设置为默认函数---接受value和key作为参数，返回value
		 * 加上---接受value和key作为参数，返回值自己定义
		 */
		cb = _.isFunction(cb) ? cb : value => value
		
		return (function fn (info) {
			let result = info
			
			/**
			 * 如果是Object---for-in设置
			 * 判断当前的obj[key]是否为数组或者对象---是则递归，反之就直接将其设置为getter的值
			 * 下面的数组同理
			 */
			if (_this.isObject(info)) {
				result = {}
				_.each(info, (value, key) => {
					Object.defineProperty(result, key, {
						value: _this.isObject(value) || _.isArray(value) ? fn(cb(value, key)) : cb(value, key),
						enumerable,
					})
				})
			}
	...
	autoToSize (size, fixNum = 0, defaultVal = '-') {
		...
		if (middleValue < Math.pow(1024, 1)) {
			return this.toFixed(size * Math.pow(1024, 0), fixNum, defaultVal)
		}
